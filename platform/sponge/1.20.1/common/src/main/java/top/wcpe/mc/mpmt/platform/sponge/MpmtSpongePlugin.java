 ...
 *
 * <p>分三步契合 Sponge 生命周期：① 构造期（{@link ConstructPluginEvent}）创建运行时；② 通道注册期
 * （{@link RegisterChannelEvent}）把注册事件、插件容器与配置目录交给 SPI，由 L4 注册 {@code mpmt:main}
 * 并装配端口/特性；③ 引擎启动期（{@link StartedEngineEvent}）再 {@code runtime.enable()} 与接线。
 * 心跳定时器依赖 {@code Sponge.server()}，不得在 RegisterChannel 阶段启用（RC1365 此时专用服尚未就绪）。
 *
...
public final class MpmtSpongePlugin {
    ...
    /** 通道注册期暂存，待引擎启动后再 enable/接线。 */
    private BanRegistry pendingBanRegistry;
    private SessionRegistry pendingSessions;
    ...
    public void onRegisterChannels(final RegisterChannelEvent event) {
        ...
        pendingBanRegistry = new BanRegistry();
        pendingSessions = new SessionRegistry();
        serverNetworkFeature =
                new ServerNetworkFeature(
                        pendingBanRegistry, () -> UUID.randomUUID().toString(), pendingSessions);
        runtime.features().register(serverNetworkFeature);
        // 不在此 enable：HeartbeatService 会 runTimer → Sponge.server()，RegisterChannel 时尚不可用
        logger.info("MPMT 通道与端口已装配，待服务端引擎启动后启用运行时");
    }

    @Listener
    public void onStarted(final StartedEngineEvent<Server> event) {
        if (runtime == null || runtime.phase() != MpmtRuntime.Phase.NEW) {
            return;
        }
        if (serverNetworkFeature == null || pendingBanRegistry == null || pendingSessions == null) {
            logger.error("MPMT 引擎启动时缺少通道装配结果，无法启用运行时");
            return;
        }
        runtime.enable();
        wireServerServices(pendingBanRegistry, pendingSessions);
        pendingBanRegistry = null;
