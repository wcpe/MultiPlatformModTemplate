...
class NeoForgeSchedulerPortTest {

    @Test
    void 调度器在停服时可释放资源() throws Exception {
        assertTrue(AutoCloseable.class.isAssignableFrom(NeoForgeSchedulerPort.class));

        NeoForgeSchedulerPort scheduler = new NeoForgeSchedulerPort(allocateServer());
        try {
    ...
    private static ExecutorService asyncPoolOf(NeoForgeSchedulerPort scheduler) throws Exception {
        Field field = NeoForgeSchedulerPort.class.getDeclaredField("asyncPool");
