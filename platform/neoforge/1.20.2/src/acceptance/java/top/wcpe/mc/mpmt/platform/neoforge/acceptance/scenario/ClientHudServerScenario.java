...
public final class ClientHudServerScenario extends ServerScenario {
    ...
    private static void sendHud(ServerGameTestContext context) {
        List<ServerPlayer> players =
                ((NeoForgeServerGameTestContext) context).server().getPlayerList().getPlayers();
        if (players.isEmpty()) {
            context.fail("无在线玩家");
            return;
        }
        NeoForgeConnectionHandle connection =
                MpmtNeoForgeMod.serverTransport().connectionFor(players.get(0));
        MpmtNeoForgeMod.serverNetworkFeature()
                .hudMessageService()
