...
1. GitHub Actions 承担远端持续集成、手动 Release、依赖审查、CodeQL 与依赖更新治理；质量构建统一走**单一根构建**的 `:buildAll`（全部平台车道为根子模块，ADR-0026）。
2. 每个需要根构建的 job 固定检出 `wcpe/mc-testkit` 的 `v0.5.1` 提交，先执行 `publishToMavenLocal`，只向 Maven Local 提供本仓限定的插件坐标。
3. 根构建以 **JDK 25** 守护运行（26.2 两条车道在配置期硬校验，ADR-0026 决策 4）；8/17/21 仅作 toolchain 目标由 foojay 解析。全部车道（含跨代 Forge 与 NeoForge）由根任务构建，**不存在**车道自有 wrapper 或"根只校验已生成制品"的旁路。
...
- ~~只运行根 `:buildAll`：无法在冷 Runner 提供 NeoForge 与跨代 Forge 所需的独立制品，否决。~~
  **该否决理由已被 [ADR-0026](0026-single-build-subproject-unification.md) 消除**：全部车道为根子模块，`buildAll` 即可完整构建，现为唯一质量构建入口。
- ~~让根 Gradle 嵌套启动独立 wrapper：违反 ADR-0007 和工具链隔离约束，否决。~~
  **约束来源已由 [ADR-0026](0026-single-build-subproject-unification.md) 取代**（车道不再有自有 wrapper，嵌套问题不复存在）。
