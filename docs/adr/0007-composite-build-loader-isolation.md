...
## 状态
已接受（**决策 2 的"加载器插件物理隔离"部分已被 [ADR-0026](0026-single-build-subproject-unification.md) 取代**：mod 平台构建插件统一为 `top.wcpe.loom` 后，平台车道一律作为根构建子模块，不再有独立构建 / 自有 wrapper / 反向 include；本 ADR 的其余决策——共享核心为根构建常规模块、Bukkit 家族按系列收敛（决策 1/3/4）——继续有效）
...
## 理由
> 以下理由记录**当时的判断**。其中"复合构建隔离相互冲突插件"的前提已被 [ADR-0025](0025-wcpe-loom-toolchain-unification.md)（插件统一）+ [ADR-0026](0026-single-build-subproject-unification.md)（子模块化）消除，见 ADR-0026 的背景与理由。
...
- 约束（写入 architecture-invariants）：~~带专属插件的平台一律独立车道；**禁止把 Loom/ForgeGradle/NeoGradle/SpongeGradle 塞进同一构建；NeoForge 1.20.2 禁止反向 include 根构建或在根任务中嵌套其 wrapper**~~ **【已被 [ADR-0026](0026-single-build-subproject-unification.md) 取代】**：现行约束为"平台车道一律为根构建子模块、构建插件统一 `top.wcpe.loom`、禁止开启 isolated projects"；**Folia 不得另起独立构建/模块，只走 FeatureGate** 继续有效。
