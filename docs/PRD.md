...
| FR-16 | 最新版本 26.2 适配（MC 今年最新版本号，新版号方案无 `1.` 前缀）：仅 Paper/Bukkit、Fabric、Forge 三个有效车道；`v26_2`、R7 三公共场景和严格报告门已实现，当前候选提交已以冻结 Paper build 71 通过同轮 `p3-r7-1787686232087` 的 `:runP3R7Gate`；依 [ADR-0023](adr/0023-p3-r7-automated-release-authority.md) 该门是最终自动化验收，冻结见 [specs/fr-26_2-adapter.md](specs/fr-26_2-adapter.md) | P3 | 开发中 |
...
- **P3 / FR-16**：仅 Paper/Bukkit 26.2、Fabric 26.2、Forge 26.2 三车道共同提供本轮 **R7**；每车道均须有属于当前运行的 `SERVER-GAMETEST-REPORT v2`、`MATRIX R7`、制品哈希、三个 required 场景各恰好一次 PASS 与末行 `RESULT PASS`。R7 源码、构建产物或历史 P2 报告均不能替代这一证据；同轮报告经 `:runP3R7Gate` 严格验证即为 FR-16 的最终自动化验收（ADR-0023）。
