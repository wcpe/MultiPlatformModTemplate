...
| FR-08 | platform-fabric 胶水（Loom）：客户端 + 服务端双端入口（锚点 1.20.1）；车道为根构建子模块（ADR-0026） | P1 | 已交付@v0.1.0 |
| FR-09 | platform-forge 胶水（Loom）：客户端 / 服务端分离代理（锚点 1.20.1）；车道为根构建子模块（ADR-0026） | P1 | 已交付@v0.1.0 |
...
| FR-32 | 远端交付治理：GitHub Actions 为模板提供全车道构建、手动 Release、依赖审查、CodeQL 与依赖更新；CI 仅作构建质量门，不替代 realserver/REALSERVER262 真服验收 | P4 | 开发中 |
...
- **平台/版本扩展验收**：每新增一个平台（P2/P3）或版本（P2/P3），其冒烟特性须在该目标真实环境通过，默认由用户确认；P3 / FR-16 的例外以 ADR-0023 为准。
- **P3 / FR-16**：仅 Paper/Bukkit 26.2、Fabric 26.2、Forge 26.2 三车道共同提供本轮 **REALSERVER262**；每车道均须有属于当前运行的 `SERVER-GAMETEST-REPORT v2`、`MATRIX REALSERVER262`、制品哈希、三个 required 场景各恰好一次 PASS 与末行 `RESULT PASS`。REALSERVER262 源码、构建产物或历史 P2 报告均不能替代这一证据；同轮报告经 `:runRealServerGate262` 严格验证即为 FR-16 的最终自动化验收（ADR-0023）。
- **P3 / FR-17、FR-18**：仓库内说明、Counter 源码或本地 release/tag 都不足以标交付；FR-17 的公开 GitHub Template 已启用，但仍须完成目标版本的 `v0.3.0` / GitHub Release。FR-18 已在干净克隆复现换名、编译与纯 JVM 测试（使用本机预热缓存）；Counter 不预置 L3 装配，使用者须按上手路径完成自己的产品接入与真服验证。三项 FR 分别验收，任一未齐均不得预标 `已交付`。
