...
- **FR-16 · 26.2 版本适配**：在已落地的 L4 `version-api` + `vX_Y` 机制（ADR-0003）下，为有效平台子集实现 `v26_2` 与 REALSERVER262 验收；当前代码、构建和真服证据必须一并闭环。
...
- FR-23 已是 P1 验收门；P3 只复用其 realserver v2 设施并新增 REALSERVER262 矩阵目录，不重新定义 FR-23 的验收语义。
...
| MC 26.2 · Fabric | 服务端 mod | Fabric 客户端产品 mod + 验收伴侣 | 根子模块（工程路径 `:platform:fabric:fabric-26.2`，目录 `platform/fabric/26.2`） |
| MC 26.2 · Forge | 服务端 mod | Forge 客户端产品 mod + 验收伴侣 | 根子模块（工程路径 `:platform:forge:forge-26.2`，目录 `platform/forge/26.2`）；`top.wcpe.loom-no-remap` 无混淆链路 |
| MC 26.2 · NeoForge / Sponge | **不属于本期**：NeoForge 仅锚 1.20.2、Sponge 与 26.2 当前不同步 | — | 见 §2.2 |

工具链冻结值：全部平台车道为**单一根构建的子模块**（ADR-0026）、Gradle 统一 **9.6.1**（ADR-0025）；26.2 两车道以 **JDK 25** 运行根构建（26.2 车道的 loom 与车道脚本在配置期硬校验），构建插件统一 `top.wcpe.loom` 1.17.1；具体坐标、哈希与命名策略以 [`fr-26_2-adapter.md`](fr-26_2-adapter.md) 为唯一权威位置。

复用现有 Gradle 入口：`:runVersionMatrixRealServerAcceptance` 不扩张 26.2，**26.2 走全 lane 的 `:runRealServerAcceptance`**（NeoForge/Sponge 不阻断的同款聚合），与 ADR-0021 / 操作手册 §3 不冲突。

## 4. 设计概述

### 4.1 FR-16 · 26.2

- 入口为三个物理车道：Bukkit `platform/bukkit/26.2`、Fabric `platform/fabric/26.2` 与 Forge `platform/forge/26.2`，三者均为**根构建子模块**（ADR-0026）。Folia、NeoForge、Sponge 不建 26.2 格。
- 三个平台按 L4 抽象挂接 `v26_2`，运行期探测 MC 版本选中该实现；26.1+ 的无混淆命名规则见 ADR-0022，而非已被取代的 ADR-0016。
- `MatrixScenarioCatalog` 已定义 REALSERVER262 的三个 required 场景：`product-handshake`、`product-roundtrip`、`client-hud`。根 `:buildRealServerArtifacts262` 与 `:runRealServerAcceptance262` 分别检查三车道产物与同一轮报告，`:runRealServerGate262` 聚合两者；报告门同时校验 `MATRIX REALSERVER262`、`RUN_ID`、制品哈希、required 场景与最终结果。当前候选提交已通过冻结 Paper build 71 的同轮 `p3-r7-1787686232087`；依 ADR-0023，该严格门是 FR-16 的最终自动化验收。
- 验收：每条 26.2 realserver lane 产生当前 REALSERVER262 的 `RESULT PASS`，同轮严格报告门通过即完成 FR-16 的最终自动化验收（ADR-0023）。
...
- **FR-16**
  - `:runRealServerAcceptance` 含 26.2 车道且 `RESULT PASS`（REALSERVER262）；归档报告存在 `MATRIX REALSERVER262 / RUN_ID / 制品哈希 / required 三场景恰好一次 PASS`。
  - 在同一 `RUN_ID` 下，使用冻结 Paper build 71 生成 Paper、Fabric、Forge 三车道当前报告，并由 `runRealServerGate262` 验证通过。
  - 同轮 REALSERVER262 报告通过 `:runRealServerGate262` 的严格验证（ADR-0023）；随后才可随发布标 `已交付@vX.Y.Z`。
- **FR-17**
