...
仓库已经是公开 GitHub Template，但尚未配置 GitHub Actions。现有的 `:buildAll`、全平台车道构建、静态检查和发布制品聚合只能在维护者本机验证，拉取请求与推送没有远端可复核的质量结果。

维护者确认将本能力作为新的治理需求实施，不追溯改变第三期 FR-16 至 FR-18 的验收语义。尤其是：P3 26.2 真服门的最终自动化证据仍由 ADR-0023 所定义的本机真实进程 Gradle 门产生，不能把 GitHub Hosted Runner 的普通构建伪装成 REALSERVER262 真服验收。

目标是在不引入凭证、不自动发布、不破坏单一根构建的前提下，使模板开箱具备可复现的远端质量、发布、安全与依赖维护工作流。
...
- 新增 `ci.yml`：对 pull request、`main`/`dev` 推送和手动触发执行完整可构建验证。
  ...
  - 明确安装 Java 8、17、21、25；以 **JDK 25 守护**执行单一根构建的 `:buildAll`（全平台车道均为根子模块，无独立 wrapper / `includeBuild` 车道；Java 8/17/21 仅供 toolchain 与矩阵轨解析）。
  - 在单一根构建内完成全车道构建后执行根 `:buildAll`，并校验 Counter、E2E harness 与 bot 模板可构建/静态检查。
...
- 将 CI/CD 入口、边界、产物与本机真服门的区别同步到 PRD、架构/运维、版本说明、README、CHANGELOG、规则索引和规格导航。

### 2.2 范围外

- 不在 GitHub Hosted Runner 运行 `:runRealServerGate262`、`runRealServerAcceptance` 或其它需真实服务端与程序化客户端的真服门；这些继续由维护者本机经 Gradle 执行。
...
- 不改变 P3 FR-16 的真服门权威，也不以 CI 成功替代任何已定义的真服证据。
...
`ci.yml` 的单一质量 job 在同一个根构建内完成全部车道构建，保证所有发布制品都来自同一 commit：

1. 构建并发布固定的 mc-testkit 到 Maven Local。
2. 安装 Java 8 / 17 / 21 / 25 并启用 Gradle 缓存；每次根 Gradle 调用均将 Loom 映射缓存显式指向当前 checkout 的绝对路径，避免 Hosted Runner 误用相对缓存目录。
3. 以 **JDK 25** 守护运行根 `:buildAll`（全车道制品 + 发布产物结构门 + `build/dist` 聚合），并把 `MPMT_JAVA8/17/21/25_HOME` 传给矩阵轨与跨代车道显式选 JDK。
...
- CI 在无预热 Maven Local 的 Hosted Runner 上固定构建 mc-testkit `0.5.1`，再以 JDK 25 守护完成单一根构建的全车道构建与 `:buildAll`（无独立车道、无嵌套 Gradle 调用）。
...
- 不存在任何自动远端 REALSERVER262 真服运行；P3 26.2 真服门的通过仍只引用本机 Gradle 证据，CI 绿不改变其验收权威。
...
- GitHub Hosted Runner 不具备本机真服验收的受控运行条件，故 REALSERVER262 真服验收必须持续留在本机 Gradle 门。
