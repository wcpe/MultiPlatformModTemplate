...
1. **阻断验证**：核心单测 + 当期约定门 + `:collectReleaseArtifacts` 全绿。P2 release 使用 `:runVersionMatrixGate`（只覆盖 STANDARD/HYBRID/SCHEDULER，**不含 26.2**）；P3 26.2 release 还必须过 `:runRealServerGate262` 的当前三车道真服证据。FR-32 启用后，候选提交还须有远端 `ci.yml` 绿色；该结果补充构建质量，不替代 26.2 真服门。
...
`collectReleaseArtifacts` 是严格聚合：仅当下列 **13** 个正式产品 jar 全部存在时才会重建 `dist`；任一缺失即失败，并保留已有 `dist`。矩阵为 Bukkit 1.12.2 / 1.20.1 / 1.21.1 / 26.2、Fabric 1.20.1 / 1.21.1 / 26.2、Forge 1.12.2 / 1.20.1 / 1.21.1 / 26.2、NeoForge 1.20.2、Sponge 1.20.1。Gradle 全车道已统一 9.6.1（ADR-0025），全部平台车道为**单一根构建的子模块**（ADR-0026）：13 个 jar 由根构建的项目依赖直接产出（如 `:platform:forge:forge-1.21.1`），任何 Gradle 调用都在仓库根执行、车道不再有自有 wrapper（见 [`OPERATIONS.md`](OPERATIONS.md)）。**根构建须以 JDK 25 守护运行**（26.2 两条车道在配置期硬校验）。聚合到 `dist` 不等于 26.2 真服验收通过。
...
| 第三期 | 26.2 + 模板发布 + 上手文档（FR-16/17/18） | FR-16 已交付 @v0.3.0；FR-17 / FR-18 待 `v0.3.0` 的 GitHub Release 完成（开发中） |
| 第四期（治理） | GitHub Actions 远端构建、手动 Release、安全与依赖维护（FR-32） | 开发中：实现与工作流已就绪，CI 只作为可复现构建质量门，真服验收仍在本机 Gradle 执行 |
