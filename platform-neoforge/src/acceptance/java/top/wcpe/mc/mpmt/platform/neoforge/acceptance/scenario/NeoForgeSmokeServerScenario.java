...
/**
 * NeoForge 冒烟验收场景（realserver，ADR-0014）：等程序化客户端连入 → 服务端经产品通道发 ACTIONBAR HUD →
 * 客户端验证器断言渲染收到。与 Forge/Fabric/Bukkit 冒烟场景同覆盖（同 HUD 文本 / 同步骤 id）。经 {@code ServiceLoader}
 * 被驱动发现。
 *
 * <p><b>经产品传输发 HUD</b>：产品通道 {@code mpmt:main} 为主 mod 注册的
 * {@link top.wcpe.mc.mpmt.platform.neoforge.net.NeoForgeServerTransport SimpleChannel}（会给消息加帧字节），故本场景
 * 取主 mod 的活跃传输 Holder、经其 {@code send} 发——既复用同一通道、又确保帧字节与客户端收包一致，且真正走产品收发链路
 * （FR-27）。
 */
public final class NeoForgeSmokeServerScenario extends ServerScenario {

    /** 等客户端连入超时：realserver NeoForge dev 客户端冷启动慢，留充足余量。 */
    ...
    public void run(ServerGameTestContext context) {
        ...
        NeoForgeServerGameTestContext neoforge = (NeoForgeServerGameTestContext) context;
        // 服务端经产品传输发一个 HUD（onMain 块内访问在线玩家）：客户端 NeoForgeHudRenderer 应渲染并记录
        context.onMain(
                () -> {
                    List<ServerPlayer> players = neoforge.server().getPlayerList().getPlayers();
                    if (!players.isEmpty()) {
                        byte[] data =
                                new PacketCodec()
                                        .encode(
                        ...
                        MpmtNeoForgeMod.activeTransport()
                                .send(new NeoForgeConnectionHandle(players.get(0)), data);
                    }
                });
