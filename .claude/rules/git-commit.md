...
- **验证门通过才提交**：`git commit` 前，本次变更必须已过验证门（判据见 `testing-and-quality.md` §1）。**门未全绿、或完成被实测推翻时，不得提交**；涉及实机 / 集成维度的，待用户确认验收通过再提交，先继续修到门过再提交；P3 FR-16 仅可按 ADR-0023 以同轮 R7 严格门通过替代该确认。
