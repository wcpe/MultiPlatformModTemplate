...
- **测试覆盖率（Java）**：`jacoco`——**全部模块**（L0–L2 + 共享 + bukkit + 各平台车道）设 LINE 覆盖率底线 0.70 门禁并入 `check`；平台车道原"仅出报告、不设底线"的豁免已废止（ADR-0026）。
...
- **本地（当前底线，强制）**：提交前自行跑静态检查（`./gradlew check` 含 Checkstyle/PMD/SpotBugs/JaCoCo/ktlint/detekt/ArchUnit；平台车道已是根构建子模块，`./gradlew :platform:<loader>:<版本>:check` 即可），违规即失败、不把问题留给后续。注：全工具一次 `check` 在低内存机器可能 OOM，可分工具 / 分模块跑。构建需 **JDK 25** 守护（26.2 车道的配置期硬校验，见 ADR-0026）。
- **CI 门禁（强制）**：`.github/workflows/ci.yml` 在 pull request、`main`/`dev` 推送与手动触发时，以固定 mc-testkit Maven Local 回退、独立 Java/Gradle 车道和 `:buildAll` 执行上述检查与测试；远端未绿不得合并。CI 不运行或替代 realserver/REALSERVER262 真服门。OWASP Dependency-Check 仍因需要 `NVD_API_KEY` 未接入。
