...
- 编排实现：`build-logic/realserver-acceptance`（`PaperHostService` + 报告门禁）、`build-logic/build-conventions` 的 `build-conventions.release`（根侧发布聚合与真服/版本矩阵门禁编排，ADR-0027）、任务图；A 车道 `top.wcpe.mc-testkit` 应用在 `:e2e:harness` 子模块（根侧只留 `runMcTestkitSmoke` / `runMcTestkitFoliaSmoke` 别名）+ `e2e/bot` 机器人。
