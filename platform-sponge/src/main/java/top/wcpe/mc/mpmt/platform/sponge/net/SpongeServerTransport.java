 ...
 *
 * <p><b>线程契约</b>：Sponge 在服务端主线程派发入站 play 数据，{@link #handle} 在主线程触发；
...
public final class SpongeServerTransport implements TransportPort {
    ...
    public SpongeServerTransport(RawDataChannel channel) {
        this.channel = Objects.requireNonNull(channel, "channel 不能为空");
        // 注册服务端 play 阶段收包处理器：仅服务端 Game 状态，可经状态取回连接玩家身份
        this.channel.play().addHandler(ServerConnectionState.Game.class, this::handle);
    }

    @Override
    public void send(ConnectionHandle connection, byte[] data) {
        UUID playerId = ((SpongeConnectionHandle) connection).playerId();
        // 按 UUID 取当前在线玩家（重连安全）；玩家已离线则静默丢弃
        Sponge.server()
                .player(playerId)
    ...
    private void handle(ChannelBuf buf, ServerConnectionState.Game state) {
        ...
        handler.accept(new SpongeConnectionHandle(player.uniqueId()), data);
    }
}
